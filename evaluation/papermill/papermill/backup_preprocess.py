...
class CustomFileNotFoundError(Exception):
    ...
    def try_fix(self, cell, source_text, input_path, repo_path):
        ...
        dataset_folder_absolute = '/datadrive/cong/datafiles/{}/'.format(self.dataset)
        ...
        print("trying for fix file missing: {}, repo_path = {}, target location = {}".format(self.file_name, repo_path, target_file))
        ...
        #for fn in os.listdir('/datadrive/cong/replay/'):
        #    if fn == filename:
        #        try:
        #            shutil.copy(os.path.join('/datadrive/cong/replay/',fn), target_file)
        #            print("mv {} {}".format(os.path.join('/datadrive/cong/replay/',fn), target_file))
        #            return True
        #        except:
        #            pass
            ...
            try:
                ...
                if os.path.exists(orig_pickle_file):
                    try:
                        ...
                        shutil.copy(orig_pickle_file, '/datadrive/cong/dumped_variables/{}_cell{}.pickle'.format(self.input_path.replace('/','_').replace('.ipynb',''), cellid))
                        fp1 = open('/datadrive/cong/cell_source_code/{}_cell{}.py'.format(self.input_path.replace('/','_').replace('.ipynb',''), cellid),'w')
    ...
    def run_cell(self, cell, cell_index=0, raise_fixable_error=False):
        ...
        while True:
            try:
                ...
                msg = self.kc.iopub_channel.get_msg(timeout=self.timeout) # None / self.iopub_timeout
            except Empty:
